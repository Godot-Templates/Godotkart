...
@export var upright_stability: float = 70.0
@export var angular_stability_damping: float = 24.0
@export var max_front_steer: float = 0.36
@export var max_lean: float = 0.34
@export var visual_smoothing: float = 10.0
@export var max_angular_speed: float = 4.0
...
@export var lean_torque: float = 680.0
@export var lean_turn_torque: float = 430.0
...
func _physics_process(delta: float) -> void:
    ...
    _apply_lean_steering(steer_input)
    _apply_balance_assist(steer_input)
    _apply_body_drag()
...
func _create_wheel_hinge(hinge_name: String, wheel_body: RigidBody3D, parent_node: Node, _motor_enabled: bool) -> HingeJoint3D:
    ...
    hinge.global_transform = _make_hinge_transform(wheel_body)
...
func _make_hinge_transform(wheel_body: RigidBody3D) -> Transform3D:
...
func _apply_lean_steering(steer_input: float) -> void:
    ...
    if absf(steer_input) > 0.01:
        ...
        apply_torque(roll_axis * steer_input * lean_torque)
    var lean_angle: float = asin(clampf(global_transform.basis.y.normalized().dot(global_transform.basis.x.normalized().cross(Vector3.UP)), -1.0, 1.0))
    var speed_factor: float = clampf(speed / 12.0, 0.0, 1.0)
    if absf(lean_angle) > 0.01 and speed > 0.5:
        apply_torque(Vector3.UP * lean_angle * lean_turn_torque * speed_factor)
    elif absf(steer_input) > 0.01:
        apply_torque(Vector3.UP * steer_input * steering_torque * 0.18)
...
func _apply_balance_assist(steer_input: float) -> void:
    var horizontal_velocity: Vector3 = Vector3(linear_velocity.x, 0.0, linear_velocity.z)
    ...
    var target_lean_angle: float = -steer_input * max_lean * speed_factor
    var target_up: Vector3 = ground_up.rotated(forward_axis, target_lean_angle).normalized()
    ...
    if correction_strength > 0.001:
        apply_torque(correction_axis.normalized() * correction_strength * upright_stability)
...
func _update_visuals(steer_input: float, delta: float) -> void:
    ...
    _visual_lean = lerpf(_visual_lean, steer_input * max_lean * speed_ratio, smoothing_weight)
    _model.rotation = _model_base_rotation + Vector3(0.0, 0.0, _visual_lean)
    _front_fork_left.rotation = _front_fork_left_base_rotation + Vector3(0.0, -steer_input * max_front_steer, 0.0)
    _front_fork_right.rotation = _front_fork_right_base_rotation + Vector3(0.0, -steer_input * max_front_steer, 0.0)
    _handlebar.rotation = _handlebar_base_rotation + Vector3(0.0, -steer_input * max_front_steer, 0.0)
    _left_grip.rotation = _left_grip_base_rotation + Vector3(0.0, -steer_input * max_front_steer, 0.0)
    _right_grip.rotation = _right_grip_base_rotation + Vector3(0.0, -steer_input * max_front_steer, 0.0)
    _headlight.rotation = _headlight_base_rotation + Vector3(0.0, -steer_input * max_front_steer, 0.0)
