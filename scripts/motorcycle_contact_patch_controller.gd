class_name MotorcycleContactPatchController
extends RigidBody3D
...
@export var engine_acceleration: float = 24.0
...
@export var max_low_speed_steer: float = 0.55
@export var max_high_speed_steer: float = 0.25
@export var steering_response: float = 12.0
@export var turn_lateral_grip: float = 13.0
@export var max_lean: float = 0.62
@export var lean_response: float = 8.0
@export var suspension_rest_length: float = 0.48
@export var suspension_stiffness: float = 12500.0
@export var suspension_compression_damping: float = 2100.0
@export var suspension_rebound_damping: float = 3100.0
@export var suspension_max_force: float = 9200.0
@export var lateral_velocity_damping: float = 18.0
@export var rolling_resistance: float = 0.045
@export var air_drag: float = 0.008
@export var aero_downforce: float = 0.24
...
func _update_ground_reference(delta: float) -> void:
    ...
    _smoothed_ground_up = _smoothed_ground_up.slerp(sampled_up, weight).normalized()
...
func _update_drive_speed(throttle_pressed: bool, brake_pressed: bool, delta: float) -> void:
    var measured_forward_speed: float = linear_velocity.dot(_bike_forward)
    _drive_speed = lerpf(_drive_speed, measured_forward_speed, clampf(delta * 5.0, 0.0, 1.0))
    if throttle_pressed:
...
func _update_steering_and_heading(steer_input: float, delta: float) -> void:
    ...
    var raw_yaw_rate: float = _drive_speed * tan(_front_steer_angle) / maxf(wheelbase, 0.1)
    var grip_yaw_limit: float = turn_lateral_grip / maxf(speed_abs, 4.0)
    ...
    var lateral_acceleration: float = speed_abs * absf(_yaw_rate)
